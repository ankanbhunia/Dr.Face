...
class SAEHDModel(ModelBase):
    ...
    def on_initialize_options(self):
        ...
        #print (device_config.devices)
        lowest_vram = 4
        if len(device_config.devices) != 0:
            lowest_vram = sum([i.total_mem_gb for i in device_config.devices])
            
        #print (lowest_vram)
        
        f4 = np.array([1280.31875, 1350.8875 , 1421.45625, 1492.025  , 1562.59375,
	       1633.1625 , 1703.73125, 1774.3    , 1844.86875, 1915.4375 ,
	       1986.00625, 2056.575  , 2127.14375, 2197.7125 , 2268.28125,
	       2338.85   , 2409.41875, 2479.9875 , 2550.55625, 2621.125  ,
	       2691.69375, 2762.2625 , 2832.83125, 2903.4    , 2973.96875,
	       3044.5375 , 3115.10625, 3185.675  , 3256.24375, 3326.8125 ,
	       3397.38125, 3467.95   , 3538.51875, 3609.0875 , 3679.65625,
	       3750.225  , 3820.79375, 3891.3625 , 3961.93125])
              
        f3 = np.array([1344.16666667, 1613., 1881.83333333, 2150.66666667,
              2419.5       , 2688.33333333, 2957.16666667, 3226.        ,
              3494.83333333, 3763.66666667, 4032.5       , 4301.33333333,
              4570.16666667, 4839.        , 5107.83333333, 5376.66666667,
              5645.5       , 5914.33333333, 6183.16666667, 6452.        ,
              6720.83333333, 6989.66666667, 7258.5       , 7527.33333333,
              7796.16666667, 8065.        , 8333.83333333, 8602.66666667,
              8871.5       , 9140.33333333, 9409.16666667])      
                
        f2 = np.array([ 2419.5,  3548.6,  4677.7,  5806.8,  6935.9,  8065. ,  9194.1,
              10323.2, 11452.3, 12581.4, 13710.5, 14839.6, 15968.7, 17097.8,
              18226.9, 19356. , 20485.1, 21614.2, 22743.3, 23872.4, 25001.5,
              26130.6, 27259.7, 28388.8, 29517.9, 30647. , 31776.1, 32905.2,
              34034.3, 35163.4, 36292.5, 37421.6])

        suggest_batch_size = 1
        
        if settings.force_batch_size == 'Auto':
          if params['Quality'] == '4': 
            
            for idx,i in enumerate(f4):
              if lowest_vram*700-i>0:
                #print (idx,i)
                suggest_batch_size = idx+1
            
            
          if params['Quality'] == '3': 
            
            for idx,i in enumerate(f3):
              if lowest_vram*700-i>0:
                #print (idx,i)
                suggest_batch_size = idx+1	
          ...
          if params['Quality'] == '1': 
            ...
            suggest_batch_size = lowest_vram//10
          ...
          f.write('suggest_batch_size '+str(suggest_batch_size)+'\n')
          f.close()
  
          
        
        else:
        	suggest_batch_size = settings.force_batch_size
        
        
        
        #print (suggest_batch_size)
        
        if suggest_batch_size == 0:sys.exit(0)
        ...
        ask_override = True#self.ask_override()
