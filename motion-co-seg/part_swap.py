...
def load_face_parser(cpu=False):
    ...
    if not cpu:
       face_parser.cuda()
       face_parser.load_state_dict(torch.load('motion-co-seg/face_parsing/cp/79999_iter.pth'))
    else:
       face_parser.load_state_dict(torch.load('motion-co-seg/face_parsing/cp/79999_iter.pth', map_location=torch.device('cpu')))
